...
from train_utils import save_model_hook, load_model_hook, get_loss, get_cosing_embeddings
...
def main():
    ...
    def preprocess_function(examples):
        queries = examples["x_title"]
        result = tokenizer(queries, padding="max_length", max_length=args.max_length, truncation=True)
        result = {f"title_{k}": v for k, v in result.items()}

        categories = examples["cat"]
        result_passage = tokenizer(categories, padding="max_length", max_length=args.max_length, truncation=True)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                title_embs = model(**{k.replace("title_", ""): v for k, v in batch.items() if "title" in k})
                category_embs = model(**{k.replace("category_", ""): v for k, v in batch.items() if "category" in k})
                prediction_scores = get_cosing_embeddings(title_embs, category_embs)
